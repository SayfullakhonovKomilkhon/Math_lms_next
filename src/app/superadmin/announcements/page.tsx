...
export default function SuperAdminAnnouncementsPage() {
  ...
  const totalReads = announcements.reduce((s, a) => s + (a.readCount ?? 0), 0);
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Всего объявлений: {data?.meta.total ?? 0} · Совокупно прочитано: {totalReads}
          </p>
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <select
          ...
          className="rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-100"
        >
